...
func newTable(verify *validate.Validator, database Database, table_name string, user_defined_schema *json.Map, schema_from_database *json.Map,  sql_command SQLCommand) (*Table, []error) {
	...
	x := Table{
		...
		Count: func(filters *json.Map, filters_logic *json.Map, order_by *json.Array, limit *uint64, offset *uint64) (*uint64, []error) {
			...
			sql_command, new_options, sql_command_errors := mysql_wrapper.GetSelectRecordsSQL(verify, table_name, *getData(), select_fields, filters, filters_logic, order_by, limit, offset, options)
			if sql_command_errors != nil {
