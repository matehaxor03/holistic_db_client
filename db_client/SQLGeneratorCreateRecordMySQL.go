...
func getCreateRecordSQLMySQL(struct_type string, table *Table, record_data *json.Map, options *json.Map) (*string, *json.Map, []error) {
	...
	record_columns, record_columns_errors := getRecordColumns(struct_type, record_data)
	...
	sql_command += ") VALUES ("
	for index, record_column := range *record_columns {
		column_data, paramter_errors := GetField(struct_type, record_data, "[schema]", "[fields]", record_column, "self")
