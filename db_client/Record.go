...
func newRecord(table Table, record_data json.Map, database_reserved_words_obj *DatabaseReservedWords, column_name_whitelist_characters_obj *ColumnNameCharacterWhitelist) (*Record, []error) {
	...
	getArchievedDate := func() (*time.Time, []error) {
		temp_value, temp_value_errors := GetField(struct_type, getData(), "[schema]", "[fields]",  "archieved_date", "*time.Time")
		if temp_value_errors != nil {
			return nil, temp_value_errors
		} else if temp_value == nil {
			return nil, nil
	...
	setLastModifiedDate := func(value *time.Time) []error {
		return SetField(struct_type, getData(), "[schema]", "[fields]", "last_modified_date", value)
	}

	setArchievedDate := func(value *time.Time) []error {
		return SetField(struct_type, getData(), "[schema]", "[fields]", "archieved_date", value)
	}
	...
	getUpdateSQL := func() (*string, *json.Map, []error) {
		...
		return getUpdateRecordSQLMySQL("*db_client.Record", temp_table, getThis(), options)
	}
	...
	created_record := Record{
		...
		GetField: func(field string, return_type string) (interface{}, []error) {
			return GetField(struct_type, getData(), "[schema]", "[fields]", field, return_type)
		},
		SetField: func(field string, value interface{}) ([]error) {
			return SetField(struct_type, getData(), "[schema]", "[fields]", field,  value)
		},
