...
func ValidateData(data *json.Map, struct_type string) []error {	
	...
	if ((struct_type == "*dao.Table" || struct_type == "dao.Table") && !data.IsBoolTrue("[schema_is_nil]"))  {
...
func ValidateParameterData(struct_type string, schemas *json.Map, schemas_type string, parameters *json.Map, parameters_type string, parameter string, value_to_validate interface{}, primary_key_count *int,  auto_increment_count *int) ([]error) {
	...
	case "*dao.Database":
		database := value_to_validate.(*dao.Database)
	...
	case "ado.Database":
		...
		break
	case "*dao..DomainName":
		domain_name := value_to_validate.(*dao.DomainName)
	...
	case "*dao.Credentials":
		credentials := value_to_validate.(*dao.Credentials)
	...
	case "*dao.DatabaseCreateOptions":
		database_create_options := value_to_validate.(*dao.DatabaseCreateOptions)
	...
	case "dao.DatabaseCreateOptions":
		...
		break
	case "*dao.Grant":
		grant := value_to_validate.(*dao.Grant)
	...
	case "dao.Grant":
		...
		break
	case "*dao.User":
		user := value_to_validate.(*dao.User)
	...
	case "dao.User":
		...
		break
	case "*dao.Table":
		table := value_to_validate.(*dao.Table)
