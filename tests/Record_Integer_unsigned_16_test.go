...
func TestRecordCanCreateRecordWithIntegerUnsigned16Column(t *testing.T) {
    ...
    record, record_errors := table.CreateRecord(class.Map{GetTestTableIntegerUnsigned16ColumnName():&test_value})
	...
	value, value_errors := record.GetUInt16(GetTestTableIntegerUnsigned16ColumnName())
	if value_errors != nil {
...
func TestRecordCanUpdateRecordWithIntegerUnsigned16Colum(t *testing.T) {
	...
	if record_errors != nil {
		...
		set_errors := record.SetUInt16(GetTestTableIntegerUnsigned16ColumnName(), &update_value)
		if set_errors != nil {
